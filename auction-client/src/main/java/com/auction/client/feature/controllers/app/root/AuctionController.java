...
public class AuctionController implements Initializable, ServerMessageListener {
    ...
    private RegisterRoleVisibilityController registerRoleVisibilityController;
    ...
    private void rebuildRouter() {
        ...
        clientResponseCoordinator = new ClientResponseCoordinator(responseRouter, responseDispatcher);
    }
    ...
    private void handleSubmitRegister() {
        if (registerFormController == null) registerFormController = new RegisterFormController(authActionFacade);
    ...
    private void handleSubmitForgotPassword() {
        if (forgotPasswordFormController == null) forgotPasswordFormController = new ForgotPasswordFormController(authActionFacade);
    ...
    public void onServerResponse(Message msg) {
        ...
        fxThreadExecutor.execute(() -> {
            ...
            clientResponseCoordinator.dispatch(msg);
        });
    }
    ...
    private void updateRegisterOrganizationVisibility() {
        if (registerRoleVisibilityController == null) {
            registerRoleVisibilityController = new RegisterRoleVisibilityController(cbRegRole, lblRegOrganization, txtRegOrganization);
        }
    ...
    public void initChart(double startPrice) {
        priceChartController = new PriceChartController(priceChart);
        priceChartController.initialize(startPrice);
    }

    public void updateChartOnNewBid(double newPrice) {
        Platform.runLater(() -> {
            if (priceChartController == null) priceChartController = new PriceChartController(priceChart);
            priceChartController.addPoint(newPrice);
        });
    }    // ==========================================================
    // AUTO BID ACTIONS
    // ==========================================================

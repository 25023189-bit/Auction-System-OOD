...
def main():
    ...
    if "start_price" in df.columns:
        price = pd.to_numeric(df["start_price"], errors="coerce")
        ratio = (values / price).replace([float("inf"), float("-inf")], pd.NA)
        ratio_summary = ratio.describe(
            percentiles=[0.01, 0.05, 0.1, 0.25, 0.5, 0.75, 0.9, 0.95, 0.99]
        ).to_frame("minimum_join_amount_to_start_price_ratio")
        ratio_summary.to_csv(REPORT_DIR / f"{COLUMN}_ratio_to_start_price.csv", encoding="utf-8-sig")
    ...
    if plt is not None:
        ...
        clean = values.dropna()
        ax.hist(clean, bins=30, color="#0891b2", edgecolor="white")
        if (clean > 0).all():
            ax.set_xscale("log")
        ax.set_title("minimum_join_amount distribution")

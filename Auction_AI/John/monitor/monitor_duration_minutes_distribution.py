...
def main():
    ...
    value_counts = values.value_counts(dropna=False).sort_index().rename("count").to_frame()
    value_counts["percent"] = (value_counts["count"] / len(df) * 100).round(2)
    value_counts.to_csv(REPORT_DIR / f"{COLUMN}_value_counts.csv", encoding="utf-8-sig")

    bins = pd.cut(
        values,
        bins=[0, 30, 60, 120, 240, 480, 720, 1440, float("inf")],
        labels=["lt_30", "30_60", "60_120", "120_240", "240_480", "480_720", "720_1440", "gt_1440"],
        right=False,
    )
    ...
    if plt is not None:
        ...
        ax.hist(values.dropna(), bins=25, color="#7c3aed", edgecolor="white")
        ax.set_title("duration_minutes distribution")
